...
namespace Lex {
    ...
    LEX lexAnaliz(Log::LOG log, In::IN in)
    {
        ...
        // !!! ÂÀÆÍÎ: Èíèöèàëèçàöèÿ íóëÿìè, ÷òîáû íå áûëî "ÌÌÌÌ" !!!
        IT::Entry entryIT = {};
        IT::Entry bufentry = {};
        LT::Entry entryLT = {};
        ...
        // Ïî óìîë÷àíèþ INT, åñëè âäðóã çàáóäåì îïðåäåëèòü
        ...
        try {
            for (int i = 0; word[i] != NULL && word[i][0] != '\0'; i++)
            {
                ...
                if (FST::execute(fstIdentif)) {
                    ...
                    entryIT = bufentry; // Ñáðîñ (òåïåðü ýòî íóëè!)
                    findFunc = false;
                    position += wordLen; continue;
                }

                // ... ËÈÒÅÐÀËÛ (Int, Hex, Bin, Str, Char) ...
                // !!! ÂÑÒÀÂÜ ÑÞÄÀ ÊÎÄ ËÈÒÅÐÀËÎÂ ÈÇ ÒÂÎÅÃÎ ÔÀÉËÀ ÈËÈ ÌÎÅÃÎ ÏÐÅÄÛÄÓÙÅÃÎ ÎÒÂÅÒÀ !!!
                // ×òîáû íå çàãðîìîæäàòü îòâåò, ÿ ñêîïèðîâàë òîëüêî íà÷àëî áëîêà.
                // Ãëàâíîå - èñïîëüçóé fstInt, fstHex, fstBin, fstStr, fstCharLit
                ...
                // --- ÑÞÄÀ ÄÎÁÀÂÜ HEX, BIN, STRING, CHAR (êàê áûëî â ïðîøëîì êîäå) ---
                ...
                // --- ÎÏÅÐÀÒÎÐÛ è ÐÀÇÄÅËÈÒÅËÈ (òîæå ñòàíäàðòíî) ---
                ...
                cout << "DEBUG: Îøèáêà 201. Ñëîâî: " << word[i] << endl;
