            ...
            for (int i = 0; word[i] != NULL && word[i][0] != '\0'; i++)
            {
                ...
                if (FST::execute(fstIdentif)) {
                    ...
                    if (idx != TI_NULLIDX) {
                        if (lex.lextable.size > 0) {
                            ...
                            if (prev == LEX_INTEGER || prev == LEX_STRING || prev == LEX_CHAR || prev == LEX_VOID) {
                                IT::Entry foundEntry = IT::GetEntry(lex.idtable, idx);
                                char currentFullName[TI_STR_MAXSIZE] = { 0 };
                                if (RegionPrefix[0] != '\0') { strcpy_s(currentFullName, RegionPrefix); strcat_s(currentFullName, (char*)word[i]); }
                                else { strcpy_s(currentFullName, (char*)word[i]); }

                                if (strcmp(foundEntry.id, currentFullName) == 0) {
                    ...
                    if (findParm && !findFunc) {
                        entryIT.idtype = IT::P; Parm_count_IT++; strcpy_s(entryIT.id, (char*)word[i]);
                    }
                    else if (!findFunc) {
                        entryIT.idtype = IT::V;
                        if (RegionPrefix[0] != '\0') { strcpy_s(entryIT.id, RegionPrefix); strcat_s(entryIT.id, (char*)word[i]); }
                        else { strcpy_s(entryIT.id, (char*)word[i]); }
                    }
                    else {
                        entryIT.idtype = IT::F;
                        strcpy_s(pastRegionPrefix, RegionPrefix); strcpy_s(RegionPrefix, (char*)word[i]);
                        strcpy_s(buferRegionPrefix, (char*)word[i]); strcpy_s(entryIT.id, (char*)word[i]);
                    }
                ...
                FST::FST fstOp(word[i], FST_OPERATOR); if (FST::execute(fstOp)) { LT::Entry lt = LT::writeEntry(entryLT, LEX_OPERATOR, indexID++, line); unsigned char op = word[i][0]; switch (op) { case '+': lt.op = LT::operations::OPLUS; lt.priority = 2; break; case '-': lt.op = LT::operations::OMINUS; lt.priority = 2; break; case '*': lt.op = LT::operations::OMUL; lt.priority = 3; break; case '/': lt.op = LT::operations::ODIV; lt.priority = 3; break; case '%': lt.op = LT::operations::OMOD; lt.priority = 3; break; case '<': if (lex.lextable.size > 0 && lex.lextable.table[lex.lextable.size - 1].lexema == LEX_COUT) { lt.op = LT::operations::OLESS; lt.priority = 1; } else { lt.op = LT::operations::OLESS_CMP; lt.priority = 1; } break; case '>': lt.op = LT::operations::OMORE; lt.priority = 1; break; case '=': lt.lexema = LEX_EQUAL; break; } LT::Add(lex.lextable, lt); strcpy_s(entryIT.id, (char*)word[i]); entryIT.idxfirstLE = lex.lextable.size - 1; entryIT.idtype = IT::OP; IT::Add(lex.idtable, entryIT); entryIT = bufentry; position += wordLen; continue; }
                ...
                for (int k = 0; k < wordLen; k++) { if ((unsigned char)word[i][k] > 127) throw ERROR_THROW_IN(206, line, position); }
                if (word[i][0] == '\'') throw ERROR_THROW_IN(207, line, position);
                cout << "DEBUG: Ошибка 201. Слово: " << word[i] << endl;
                throw ERROR_THROW_IN(201, line, position);
            }
            if (count_main == 0) { Log::WriteErrors(log, Error::geterror(301)); throw Error::geterror(301); }
            if (count_main > 1) { Log::WriteErrors(log, Error::geterror(302)); throw Error::geterror(302); }
        }
